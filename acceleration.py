...
try:
    with connection.cursor() as cursor:
        ...
        sql = f"SELECT * FROM gps_1001 order by cur_time limit 500000"
        cursor.execute(sql)
    ...
    for line in result:
        ...
        orders[order_id].append((cur_time,x + x_axis_offset,y + y_axis_offset,line_id))
...
for order in orders:
    ...
    ids = [t[3] for t in orders[order]]

    if order_cnt%100 ==0:
        print(str(order_cnt)+'/'+str(order_total))
    order_cnt+=1
